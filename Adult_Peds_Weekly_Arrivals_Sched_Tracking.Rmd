...
```{r Subset data and create custom function for visualizing schedule over next 7 days}
...
today_to_7days_templ <- data.frame("ApptDate" = rep(today_to_7days,
                                          length(all_sites)))

site_7day_sched_templ <- data.frame("Site" = sort(rep(all_sites,
                                                        length(today_to_7days)
                                                        )
                                                    ),
                                      "ApptDate" = today_to_7days_templ)

site_vax_dose_7day_sched_templ <- left_join(site_vax_dose_templ,
                                            site_7day_sched_templ,
                                            by = c("Site" = "Site"))

sch_visits <- left_join(site_vax_dose_7day_sched_templ,
                          sch_visits,
                          by = c("Site" = "Site",
                                 "VaxType" = "VaxType",
                                 "Dose" = "Dose",
...
sch_visits_kable <- function(vax_type, dose) {
  if (!(vax_type %in% c("Adult", "Peds") |
        dose %in% c(1, 2, 3, "All"))) {
    msg <- "Invalid argument"
    stop(msg)
    ...
    kable(sch_visits_kable_df, format = "html", escape = FALSE,
          align = "c", digits = 0,
          caption = paste0(vax_type, " ", dose_title, ": Scheduled Appointments")) %>%
      kable_styling(bootstrap_options = "hover", position = "center",
                    font_size = 11, full_width = FALSE) %>%
      column_spec(column = c(1,
...
sch_visits_kable(vax_type = "Adult", dose = "All")
...
sch_visits_kable(vax_type = "Adult", dose = 1)
...
sch_visits_kable(vax_type = "Adult", dose = 2)
...
sch_visits_kable(vax_type = "Adult", dose = 3)
...
sch_visits_kable(vax_type = "Peds", dose = "All")
...
sch_visits_kable(vax_type = "Peds", dose = 1)
...
sch_visits_kable(vax_type = "Peds", dose = 2)
