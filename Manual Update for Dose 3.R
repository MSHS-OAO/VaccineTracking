...
sched_breakdown <- left_join(site_dose_date_templ,
                             sched_breakdown,
                             by = c("Dose" = "Dose",
                                    # "PodType" = "Pod Type",
                                    "Site" = "Site",
...
sched_breakdown_cast <- sched_breakdown %>%
  pivot_wider(id_cols = c(Dose, Site),
...
city_sites_sched <- city_sites_sched %>%
  ...
  mutate(ExpSchMfg = ifelse(Status2 != "Sch", NA,
                            ...
                            ifelse(Site == "MSVD", "J&J",
                                   # Assume all scheduled Dose 1 and Dose 3 appointments receive Pfizer
                                   ifelse(Dose %in% c(1, 3), "Pfizer",
...
pfizer_templ <- site_dose_date_templ %>%
...
admin_to_date_mfg_test <- sched_to_date %>%
  filter(Status2 == "Arr" &
           ApptDate < today &
           Site %in% city_sites) %>%
  mutate(DateRange = ifelse(ApptDate == today - 1,
                            format(today - 1, "%m/%d/%y"),
                            paste0("Admin 12/15/20-",
                                   format(today - 2, "%m/%d/%y")))) %>%
  ...
  ungroup() %>%
  arrange(Site, Dose, desc(Mfg), desc(DateRange)) %>%
  select(Dose, Mfg, Site, DateRange, Count) %>%
  pivot_wider(names_from = DateRange,
              names_sort = FALSE,
              values_from = Count)

admin_date_ranges <- c(format(today - 1, "%m/%d/%y"),
                       paste0("Admin 12/15/20-",
                              format(today - 2, "%m/%d/%y")))
...
rep_date_range <- data.frame("DateRange" = rep(admin_date_ranges,
                                               length(unique(city_sites_sched$Site))))
...
export_list <- list("SchedSummary" = sched_summary,
                    ...
                    "CumDosesAdministered" = admin_doses_site_pod,
