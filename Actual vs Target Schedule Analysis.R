...
if (update_repo) {
  ...
  saveRDS(sched_repo, paste0(user_directory, "/Target Actual Sched Repo ",
...
sched_to_date <- sched_to_date %>%
  mutate(Status = ifelse(`Appt Status` %in% c("Arrived", "Comp", "Checked Out"), "Arr", `Appt Status`), # Group various arrival statuses as "Arr"
         Status2 = ifelse(ApptDate <= today & Status == "Sch", "No Show", Status), # Categorize any appts still in sch status from earlier today and prior days as no shows
...
sched_breakdown <- sched_to_date %>%
  filter(Status2 %in% c("Arr", "Sch") & ApptDate >= (today - 1) & ApptDate <= (today + 7)) %>%
...
# Summarize and format schedule for next 4 days for easier analysis --------------------------
...
sched_7days_dates <- rep(seq(min(sched_7days_summary$ApptDate), 
                         max(sched_7days_summary$ApptDate),
...
export_list <- list("SchedSummary" = sched_summary,
                    ...
                    "SchedFormat_Next7Days" =  sched_7days_format_jm,
                    "CumDosesAdministered" = admin_doses_table_export,
                    "Dose1Arrivals_Past7Days" = cast_dose1_7day_all_arr,
                    "Dose1WalkIns_Past7Days" = cast_dose1_7day_walkins_arr,
                    "WalkInsStats_7Days" = cast_dose1_7day_walkins_stats)

write_xlsx(export_list, path = paste0(user_directory, "/Target Actual Sched Export ", format(Sys.Date(), "%m%d%y"), ".xlsx"))
