...
colnames(mssn_admin) <- c("ApptDate", "Dose1", "Dose2", "AllDoses", "Pod Type")
```
...
# Create a template of all sites, vaccine dates, doses, and pod types ---------
...
site_dose_templ <- cbind(site_rep, dose_rep)

site_dose_templ <- site_dose_templ %>%
  mutate(Site = factor(Site, levels = all_sites, ordered = TRUE)) %>%
  arrange(Site, Dose)

pod_rep <- data.frame("Pod Type" = rep(pod_type,
                                       length(all_sites)))

site_pod_rep <- data.frame("Site" = sort(rep(all_sites,
                                             length(pod_type))))

site_pod_templ <- cbind(site_pod_rep, pod_rep)

site_pod_templ <- site_pod_templ %>%
  mutate(Site = factor(Site, levels = all_sites, ordered = TRUE)) %>%
  arrange(Site, Pod.Type)

site_dose_pod_templ <- left_join(site_dose_templ, site_pod_templ,
                                 by = c("Site" = "Site"))
...
site_dose_this_week_prior_days_templ <- left_join(site_dose_pod_templ,
...
site_dose_next_7days_templ <- left_join(site_dose_pod_templ,
...
admin_to_date2 <- daily_sched_summary %>%
  filter(ApptDate <= today) %>%
  group_by(Site, `Pod Type`, Dose) %>%
  summarize(AdminDoses = sum(Count, na.rm = TRUE),
            .groups = "keep") %>%
...
prior_weeks_admin <- daily_sched_summary %>%
  filter(VaccWeek < this_week) %>%
  group_by(Site, `Pod Type`, Dose, VaccWeek, Dates) %>%
  summarize(AdminDoses = sum(Count, na.rm = TRUE),
            .groups = "keep") %>%
  ungroup() %>%
...
this_week_daily_admin <- daily_sched_summary %>%
  filter(VaccWeek == this_week &
           ApptDate < today) %>%
  group_by(Site, `Pod Type`, Dose, ApptDate) %>%
  summarize(AdminDoses = sum(Count, na.rm = TRUE),
            .groups = "keep") %>%
  ungroup()
